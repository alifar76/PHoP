...
def picrustRun(filenames):
	...
	commands_out = ['normalize_by_copy_number.py -i %s.biom -o normalized_%s.biom','predict_metagenomes.py -i normalized_%s.biom -o metagenome_predictions_%s.biom','categorize_by_function.py -i metagenome_predictions_%s.biom -c "KEGG Pathways" -l 3 -o func_level3_%s.biom']
...
def biomConvert(val):
	...
	for line in p.stdout:
		converter = 'convert_biom.py -i %s -o %s.txt -b' % (line.strip(),line.strip().split(".")[0])
